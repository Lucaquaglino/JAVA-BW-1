...
//ELENCO SHOPS ----------------------------------------------
	public void shopList() {
		List<Punti_vendita> list = em.createQuery("SELECT shop FROM Punti_vendita shop", Punti_vendita.class)
				.getResultList();
...
//ELENCO SHOPS by LOCATION ----------------------------------------------
	public void shopListByLocation(String location) {
		List<Punti_vendita> list = em
				.createQuery("SELECT shop FROM Punti_vendita shop WHERE shop.location = :location", Punti_vendita.class)
			...
			if (shop != null) {
				if (shop instanceof Distributori) {
					...
					if (distributor.getState() == State.OUTOFSERVICE) {
						System.out.println("Impossibile emettere il ticket. Lo shop non e attivo.");
					} else {
...
//EMETTI ABBONAMENTO ---------------------------------------
		...
		try {
			if (shop != null && card != null) {
				if (shop instanceof Distributori) {
					...
					// ----
					if (distributor.getState() == State.OUTOFSERVICE) {
						System.out.println("Impossibile effettuare l'ammonamento. Lo shop non e attivo.");
					} else {
						if (card.isValid()) {
							...
							em.getTransaction().commit();
							System.out.println(
									"L'abbonamneto e stato emesso correttamente con codice: " + sub.getProductId());
						} else {
							System.out.println("La tua card non e attiva, devi rinnovarla");
						}
					}
					// ----
				} else {
	...
	public void checkSubscriptionValidity(Long cardId) {
		...
		if (!subscriptions.isEmpty()) {
			...
			if (subscription.getIsActive()) {
				System.out.println("L'abbonamento e attivo");
			} else {
				System.out.println("Purtroppo l'abbonamento e scaduto, fanne uno nuovo");
			}
		} else {
	...
	public void printTicketsAndPeriodByShopAndDate(String shopId1, LocalDate startDate, LocalDate endDate) {
		try {
			...
			System.out.println("Errore: Il formato dell'ID del punto vendita non e valido.");
